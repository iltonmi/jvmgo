...
func newMethod(class *Class, cfMethod *classfile.MemberInfo) *Method {
	...
	mdesc := parseMethodDescriptor(method.descriptor)
	method.calcArgSlotCount(mdesc.parameterTypes)
	if method.IsNative() {
		//如果是本地方法, 则注入字节码和其他信息
		method.injectCodeAttribute(mdesc.returnType)
	}
	return method
}
...
func (self *Method) injectCodeAttribute(returnType string) {
	...
	self.maxLocals = self.argSlotCount //本地方法的局部变量表只用来存放参数
	//第一条指令是预留指令,第二条指令根据函数的返回值选择
	switch returnType[0] {
	case 'V':
